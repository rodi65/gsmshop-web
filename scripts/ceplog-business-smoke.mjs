...
assert(style.includes("Targeted cart popup gap-only fix") && style.includes("height: fit-content !important") && style.includes("cart-checkout-list + .cart-final-summary"), "Sepet ürün listesi ile toplam/ödeme kartı arasındaki dikey boşluk hedefli olarak sıfırlanmalı.");
