...
assert(style.includes("Kasa dashboard final yerleşim") && style.includes("Kasa özet kartları görünürlük düzeltmesi") && style.includes("grid-template-rows: 196px minmax(300px, auto)") && style.includes("repeat(5, minmax(0, 1fr))"), "Ana Kasa dashboard özet kartları yukarı alınmış ve Kart/Cari satırları görünür kalmalı.");
