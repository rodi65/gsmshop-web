...
assert(style.includes("Kasa dashboard final yerleşim") && style.includes("Kasa geniş işlem butonları ve uzayan Kart/Cari özeti") && style.includes("grid-template-rows: 210px minmax(340px, auto)") && style.includes("repeat(5, minmax(124px, 1fr))"), "Ana Kasa dashboard yatay işlem butonları okunur ve Kart/Cari kartı uzatılmış olmalı.");
