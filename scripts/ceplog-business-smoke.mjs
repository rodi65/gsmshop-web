...
assert(cartPanel.includes("cart-product-index") && cartPanel.includes("cart-payment-section-head"), "Sepet satırları numaralı ve ödeme bölümü başlıklı olmalı.");
assert(cartPanel.includes("Parçalı ödeme girebilirsin"), "Ödeme ekranı parçalı ödeme kullanımını açıkça belirtmeli.");
