...
export default function App() {
  ...
  const [dashboardTheme, setDashboardTheme] = useState(() => {
    ...
    return ["1", "2", "3"].includes(localStorage.getItem("ceplog_dashboard_theme")) ? localStorage.getItem("ceplog_dashboard_theme") : "1";
  });
...
const isSameSalesListDay = (item, dateKey) => {
  ...
  function applyDashboardTheme(theme) {
    const nextTheme = ["1", "2", "3"].includes(String(theme)) ? String(theme) : "1";
    setDashboardTheme(nextTheme);
    if (typeof window !== "undefined") localStorage.setItem("ceplog_dashboard_theme", nextTheme);
  }
        ...
        {active === "yonetim" && (
          <section className="section management-section compact-management-section">
            ...
            <div className="management-tools-compact-grid">
              ...
              <div className="card management-card compact-tool-card management-theme-card">
                <h2>Tema</h2>
                <div className="management-theme-actions" aria-label="Dashboard tema seçimi">
                  {["1", "2", "3"].map((theme) => (
                    <button
                      key={theme}
                      type="button"
                      className={dashboardTheme === theme ? "active" : ""}
                      onClick={() => applyDashboardTheme(theme)}
                    >
                      {theme}
                    </button>
                  ))}
