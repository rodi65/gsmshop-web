...
const saleSearchGroupMeta = [
  { key: "telefon", title: "Telefon", detail: "IMEI, barkod, marka ve model ile cihaz bul." },
  { key: "aksesuar", title: "Aksesuar", detail: "Kılıf, ekran koruma, şarj ve diğer aksesuarlar." },
  { key: "teknik", title: "Teknik", detail: "Teknik servisle ilişkili cihaz ve elektronik grupları." },
  { key: "digerleri", title: "Diğerleri / Geri Kalanlar", detail: "Program, saat, tablet, PC, bluetooth ve diğer stoklar." },
];

function saleSearchGroupKey(product) {
  const group = stockSearchGroup(product);
  if (group === "TELEFON") return "telefon";
  if (group === "AKSESUAR") return "aksesuar";
  if (["ELEKTRONİK", "PC"].includes(group)) return "teknik";
  return "digerleri";
}
...
const isSameSalesListDay = (item, dateKey) => {
  ...
  const stockSearchResultGroups = useMemo(() => saleSearchGroupMeta.map((group) => ({
    ...group,
    items: stockSearchResults.filter((product) => saleSearchGroupKey(product) === group.key),
  })), [stockSearchResults]);
  ...
  function transferProductToSale(product) {
    ...
    setSyncMessage(`${title} satış penceresine aktarıldı.`);
  }
            ...
            <div className="modal search-modal">
              <div className="search-panel">
                ...
                {!stockSearchResults.length ? (
                  ...
                  <div className="search-results-table grouped-search-results">
                    {stockSearchResultGroups.map((resultGroup) => (
                      <section key={resultGroup.key} className="search-result-group" aria-labelledby={`search-group-${resultGroup.key}`}>
                        <div className="search-result-group-head">
                          <div>
                            <h3 id={`search-group-${resultGroup.key}`}>{resultGroup.title}</h3>
                            <p>{resultGroup.detail}</p>
                          </div>
                          <span>{resultGroup.items.length} ürün</span>
                        </div>

                        {resultGroup.items.length ? (
                          <div className="search-result-group-list">
                            {resultGroup.items.map((product) => {
                              const group = stockSearchGroup(product);
                          ...
                          <div className="search-result-group-empty">Bu grupta eşleşen ürün yok.</div>
                        )}
                      </section>
                    ))}
                  </div>
                ...
                <div className="kasa-layout">
                  <div className="card pad kasa-sale-card kasa-sorgula-card">
                    <button className="kasa-sorgula-launch" type="button" onClick={openSaleSearchModal}>
                      <span className="kasa-sorgula-icon"><Search size={34} strokeWidth={2.6} /></span>
                      <span className="kasa-sorgula-copy">
                        <small>Satış başlangıcı</small>
                        <strong>SORGULA</strong>
                        <em>IMEI, barkod, ürün adı, model veya cari bilgisiyle ürünü bul; mevcut satış penceresine aktar.</em>
                      </span>
                    </button>
                    <div className="kasa-sorgula-groups" aria-label="Sorgula ürün grupları">
                      {saleSearchGroupMeta.map((group) => (
                        <span key={group.key}>{group.title}</span>
                      ))}
                    </div>
                    <div className="kasa-sale-hint kasa-sorgula-hint">
                      <span>Manuel satış formu kaldırıldı.</span>
                      <b>Satış, mevcut sepet ve ödeme kurallarıyla SORGULA üzerinden devam eder.</b>
                    </div>
                  </div>
