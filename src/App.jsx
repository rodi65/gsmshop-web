...
const isSameSalesListDay = (item, dateKey) => {
  ...
  function createCartSalePayload({ rows, idempotencyKey, cartCustomerId, cartCustomerName, cartCashAmount, cartCardAmount, cartBankAmount, cartCariAmount, cartTotalAmount }) {
    const subtotal = rows.reduce((total, item) => total + (Number(item.unitPriceAtSale || 0) * Number(item.quantity || 0)), 0);
    const discount = rows.reduce((total, item) => total + Number(item.discountAmount || 0), 0);
    const paymentTotal = cartCashAmount + cartCardAmount + cartBankAmount + cartCariAmount;
    return {
      ...
      customer_id: cartCustomerId || cartCustomerName || null,
      customerName: cartCustomerName,
      customer_name: cartCustomerName,
      activeCariId: cartCustomerId || null,
      active_cari_id: cartCustomerId || null,
      activeCustomer: { customerId: cartCustomerId || null, customerName: cartCustomerName },
      active_customer: { customer_id: cartCustomerId || null, customer_name: cartCustomerName },
      activeBankId: cartBankName || "",
      active_bank_id: cartBankName || "",
      bankId: cartBankName || "",
      bank_id: cartBankName || "",
      ...
      items: rows.map((item) => ({
        ...
        unitPrice: Number(item.unitPriceAtSale || 0),
        unit_price: Number(item.unitPriceAtSale || 0),
      ...
      totals: {
        ...
        sale_total: cartTotalAmount,
        paymentTotal,
        payment_total: paymentTotal,
        remainingAmount: cartTotalAmount - paymentTotal,
        remaining_amount: cartTotalAmount - paymentTotal,
      },
      note: cartNote,
      metadata: {
        source: "cart",
  ...
  async function completeCartSale() {
    ...
    try {
      ...
      const cartSalePayload = createCartSalePayload({
        rows,
        idempotencyKey,
        cartCustomerId,
        cartCustomerName,
      ...
      if (import.meta.env.DEV) {
        console.debug("CEPLOG cartSalePayload", {
          saleTotal: cartSalePayload.totals.saleTotal,
          paymentTotal: cartSalePayload.totals.paymentTotal,
          cashAmount: cartSalePayload.payments.cashAmount,
          cardAmount: cartSalePayload.payments.cardAmount,
          cariAmount: cartSalePayload.payments.cariAmount,
          itemCount: cartSalePayload.items.length,
          itemUnitPrices: cartSalePayload.items.map((item) => item.unitPrice),
          hasBank: Boolean(cartSalePayload.bankName),
