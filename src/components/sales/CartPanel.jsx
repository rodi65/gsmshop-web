    ...
    <article className="cart-checkout-item-card">
      <div className="cart-checkout-item-main">
        <div className="cart-product-title-line">
          <span className="cart-product-index">#{Number(index || 0) + 1}</span>
...
export function CartPaymentBox({
  ...
  return (
    <div className="cart-payment-box cart-final-payment-box">
      <div className="cart-payment-section-head">
        <div>
          <strong>Ödeme Bilgileri</strong>
          <span>Parçalı ödeme girebilirsin: nakit + kart + cari toplamı sepet toplamıyla eşleşmeli.</span>
        </div>
        <b>{paymentGap === 0 ? "Dengede" : paymentGap > 0 ? "Eksik ödeme" : "Fazla ödeme"}</b>
      </div>
  ...
  return (
    <aside className="card pad kasa-cart cart-panel">
      <div className="top-line cart-top-line">
        <div>
          <h2>Satış Sepeti</h2>
          <p className="cart-panel-subtitle">{items.length ? `${items.length} ürün hazır, ödeme bilgilerini tamamla.` : "Ürün ara veya hızlı satıştan ekle."}</p>
        </div>
