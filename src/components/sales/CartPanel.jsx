...
export function CartPaymentBox({
  ...
  const statusText = paymentGap === 0 ? "Ödeme dengede" : paymentGap > 0 ? "Eksik ödeme var" : "Fazla ödeme var";
    ...
    <div className="cart-payment-box cart-final-payment-box">
      ...
      {paymentGap !== 0 && (
        <div className={`cart-payment-gap ${gapTone}`}>
          {paymentGap <= 0 && <span>Fazla ödeme</span>}
          <b>{money(Math.abs(paymentGap))}</b>
          {paymentGap > 0 ? (
            <button className="cart-cari-fill-btn" type="button" onClick={() => onPaymentChange("cariAmount", String(paymentNumber(payments.cariAmount) + paymentGap))}>
              Kalan {money(paymentGap)} cariye yaz
            </button>
          ) : (
...
export default function CartPanel({
  ...
  return (
    <aside className="card pad kasa-cart cart-panel">
      ...
      <div className="cart-section-label cart-summary-label"><span>Toplamlar</span><b>Nakit + Kart + Cari = Sepet Toplamı</b></div>
