    ...
    <div className="cart-payment-box cart-final-payment-box">
      ...
      {paymentGap !== 0 && (
        <div className={`cart-payment-gap ${gapTone}`}>
          {paymentGap > 0 ? (
            <button type="button" className="cart-write-cari-btn" onClick={() => onPaymentChange("cariAmount", String(paymentNumber(payments.cariAmount) + paymentGap))}>
              Kalanı cariye yaz: {money(Math.abs(paymentGap))}
            </button>
          ) : (
