...
const normalizeCartText = (value) => String(value || "").replace(/\s+/g, " ").trim();

function cleanCartProductName(name) {
  const cleanName = normalizeCartText(name);
  if (!cleanName.includes("/")) return cleanName;
  const parts = cleanName.split("/").map((part) => normalizeCartText(part)).filter(Boolean);
  if (!parts.length) return cleanName;
  if (parts.length % 2 === 0) {
  ...
  const compact = [];
  parts.forEach((part) => {
    if (compact[compact.length - 1]?.toLocaleLowerCase("tr-TR") !== part.toLocaleLowerCase("tr-TR")) compact.push(part);
  });
  return compact.join(" / ");
}

function normalizeCartCategory(label) {
  const cleanLabel = normalizeCartText(label);
  const lower = cleanLabel.toLocaleLowerCase("tr-TR");
  if (!cleanLabel) return "Ürün";
  if (lower.includes("akses") || lower.includes("accessory")) return "Aksesuar";
  if (lower.includes("telefon") || lower.includes("phone")) return "Telefon";
  if (lower.includes("program")) return "Program";
  if (lower.includes("servis") || lower.includes("teknik") || lower.includes("hizmet")) return "Teknik / Hizmet";
  return cleanLabel.replace(/\s*\([^)]*\)\s*/g, "").trim() || "Ürün";
}
...
export function CartItemRow({ item, money, onUpdate, onRemove }) {
  ...
  const displayName = cleanCartProductName(item.productName);
  const categoryLabel = normalizeCartCategory(item.productTypeLabel || item.category || item.productType);
  const detailText = [categoryLabel, item.imei ? `IMEI: ${item.imei}` : item.barcode ? `Barkod: ${item.barcode}` : ""].filter(Boolean).join(" • ");
...
export function CartPaymentBox({
  ...
  return (
    <div className="cart-payment-box cart-final-payment-box">
      ...
      <div className="cart-payment-actions compact-fill-actions" aria-label="Hızlı ödeme doldur">
