...
const normalizeCartText = (value) => String(value || "").trim().replace(/\s+/g, " ");
const normalizeCartCompare = (value) => normalizeCartText(value).toLocaleLowerCase("tr-TR");
const cleanCartProductName = (value) => {
  const text = normalizeCartText(value);
  if (!text) return "Ürün";
  const parts = text.split("/").map((part) => normalizeCartText(part)).filter(Boolean);
  if (parts.length >= 4 && parts.length % 2 === 0) {
    const half = parts.length / 2;
    const first = parts.slice(0, half).map(normalizeCartCompare).join("|");
    const second = parts.slice(half).map(normalizeCartCompare).join("|");
    if (first === second) return parts.slice(0, half).join(" / ");
  }
  const deduped = parts.filter((part, index) => normalizeCartCompare(part) !== normalizeCartCompare(parts[index - 1]));
  return deduped.length ? deduped.join(" / ") : text;
};
const cleanCartTypeLabel = (value) => {
  const clean = normalizeCartText(value);
  const lower = clean.toLocaleLowerCase("tr-TR");
  if (["accessory", "aksesuar", "aksesauar"].includes(lower)) return "Aksesuar";
  if (["phone", "telefon"].includes(lower)) return "Telefon";
  if (["service", "program / hizmet", "hizmet"].includes(lower)) return "Program / Hizmet";
  return clean;
};
...
export function CartItemRow({ item, money, onUpdate, onRemove }) {
  ...
  const typeLabel = cleanCartTypeLabel(item.productTypeLabel);
  const detailText = [typeLabel, item.imei ? `IMEI: ${item.imei}` : item.barcode ? `Barkod: ${item.barcode}` : ""].filter(Boolean).join(" • ");
...
export function CartPaymentBox({
  ...
  return (
    <div className="cart-payment-box cart-final-payment-box">
      ...
      <div className="cart-payment-actions cart-payment-quick-fill" aria-label="Hızlı ödeme doldur">
